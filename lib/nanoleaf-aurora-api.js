...
const AuroraApi = (module.exports = function (options) {
	...
	this.makeGetRequestOptions = function (pathname) {
		const options = this.makeOptions(pathname, 'GET');
	...
	this.makePutRequest = function (pathname, body) {
		const options = this.makeOptions(pathname, 'PUT');
	...
	this.doRequest = function (requestOptions) {
		return new Promise((resolve, reject) => {
			const request = http.request(requestOptions.options, response => {
				let chunkedData = '';
	...
	this.subscribeSSE = function (callback) {
		return new Promise((resolve, reject) => {
			this.SSEEventSource = new EventSource(
				`${this.protocol}//${this.host}:${this.port}${this.base}${this.accessToken}/events?id=1,3,4`,
			);

			this.SSEEventSource.onopen = function (_event) {
				resolve();
			};
			...
			this.SSEEventSource.onerror = function (error) {
				const errorMessage = `${error.type}: ${error.message} (status: ${error.status})`;
				if (reject) {
					reject(errorMessage);
				} else {
...
AuroraApi.getAuthToken = function (address, port) {
	return new Promise((resolve, reject) => {
			...
			switch (statusCode) {
				case 200:
					if (!/^application\/json/.test(contentType)) {
						reject({
							errorCode: 'ErrorJSON',
							message: 'Error obtaining authorization token!',
							messageDetail: `Invalid content-type. Expected "application/json" but received ${contentType}`,
						});
						return;
					}
					break;
				case 401:
					reject({
						...
						message:
							'Getting authorization token failed because access is unauthorized (is the device in pairing mode?)',
					});
					return;
				case 403:
					reject({
						errorCode: 'ErrorUnauthorized',
						message:
							'Getting authorization token failed because permission denied (is the device in pairing mode?)',
					});
					return;
				default:
					reject({
						errorCode: 'ErrorConnection',
						message: `Connection to "${address}:${port}" failed: HTTP status code ${statusCode}`,
					});
					return;
			}
			...
			res.on('end', () => {
				try {
					...
					if (parsedData['auth_token']) {
						...
						reject({
							errorCode: 'NoAuthTokenFound',
							message: 'No authorization token found!',
							messageDetail: 'JSON response does not contain an "auth_token"',
						});
					}
				} catch (err) {
					reject({
						errorCode: 'NoAuthTokenFound',
						message: 'No authorization token found!',
						messageDetail: `Error JSON parsing received data: ${err}`,
					});
				}
			});
		});
		...
		req.on('error', err => {
			reject({
				errorCode: 'ErrorConnection',
				message: `Connection to "${address}:${port}" failed, ${err}`,
			});
		});
